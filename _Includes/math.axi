 ...
 *
 * $Id: math.axi 38 2010-09-28 07:05:37Z kim.john.burgess $
...
char BASE64_ENCODE_LOOKUP_TABLE[64] = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
...
/**
 * Checks if a string value contains only a possibly signed numeric value.
 *
 * @param	a		a string to check
 * @return			a boolean, true is a is numeric
 */
...
define_function integer base64_encode(char to_encode[], char encoded[], line_length)
{
	...
	for (i = 1; i <= length_string(to_encode); i = i + 3) {
		...
		enc = "BASE64_ENCODE_LOOKUP_TABLE[(work[1] >> 2) + 1],
				BASE64_ENCODE_LOOKUP_TABLE[(((work[1] & $03) << 4) | (work[2] >> 4)) + 1],
				BASE64_ENCODE_LOOKUP_TABLE[(((work[2] & $0f) << 2) | (work[3] >> 6)) + 1],
				BASE64_ENCODE_LOOKUP_TABLE[(work[3] & $3f) + 1]"
...
define_function integer base64_decode(char to_decode[65532], char decoded[49419])
{
	...
	for (i = 1; i <= length_string(to_decode); i++) {
		for (j = 1; j <= 64; j++) {
			if (to_decode[i] == BASE64_ENCODE_LOOKUP_TABLE[j]) {
...
define_function integer base64_concat(char str1[], char str2[], char output[])
{
