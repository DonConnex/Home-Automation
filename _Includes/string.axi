 ...
 *
 *
 * $Id: string.axi 37 2010-09-06 05:15:18Z kim.john.burgess $
...
define_constant
#if_not_defined STRING_RETURN_SIZE_LIMIT
		...
		integer ret_len, char quote)
{
    ...
    while (start <= length_string(a)) {
		...
		if (end) {								// then separate by delimiter
...
/**
 * Replace the first instance of a basic string with another string.
 *
 ...
 */
define_function char[STRING_RETURN_SIZE_LIMIT] string_replace_first(char src[],
	char find[], char replace[])
{
	stack_var integer a

	a = find_string(src, find, 1)
	if (a) {
		return "left_string(src, a - 1), replace,
			mid_string(src, a + length_string(find), STRING_RETURN_SIZE_LIMIT)"
