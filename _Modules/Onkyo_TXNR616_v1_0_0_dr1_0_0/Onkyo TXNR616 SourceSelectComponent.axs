...
(*********************************************************************)
MODULE_NAME = 'Onkyo TXNR616 SourceSelectComponent' (dev vdvDev[], dev dvTP, INTEGER nDevice, INTEGER nPages[])
(***********************************************************)
(* System Type : NetLinx                                   *)
...
BTN_SOURCE_TAPE1                = 34    // Button: Tape 1
BTN_SOURCE_AUX1                 = 39    // Button: Aux
BTN_SOURCE_CD1                  = 36    // Button: CD
BTN_SOURCE_TAPE2                = 35    // Button: Tape 2
BTN_SOURCE_TUNER1               = 37    // Button: Tuner
BTN_SOURCE_TV1                  = 30    // Button: TV
BTN_SOURCE_VIDEO1               = 31    // Button: Video 1
BTN_SOURCE_VIDEO2               = 32    // Button: Video 2
BTN_SOURCE_VIDEO3               = 33    // Button: Video 3
BTN_SOURCE_PHONO1               = 38    // Button: Phono
...
//                   SourceSelectComponent: channel button - momentary channel
//                   on BTN_SOURCE_VIDEO1
//
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_VIDEO1]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_VIDEO1]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_VIDEO1),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_TAPE1]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_TAPE1]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_TAPE1),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_AUX1]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_AUX1]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_AUX1),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_CD1]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_CD1]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_CD1),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_TAPE2]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_TAPE2]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_TAPE2),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_TV1]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_TV1]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_TV1),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_VIDEO2]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_VIDEO2]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_VIDEO2),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_VIDEO3]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_VIDEO3]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_VIDEO3),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_PHONO1]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_PHONO1]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_PHONO1),']'")
        }
    }
}
...
//
//---------------------------------------------------------------------------------
BUTTON_EVENT[dvTP, BTN_SOURCE_TUNER1]
{
    push:
    {
        if (bActiveComponent)
        {
            on[vdvDev[nCurrentZone], SOURCE_TUNER1]
            println (" 'on[',dpstoa(vdvDev[nCurrentZone]),', ',itoa(SOURCE_TUNER1),']'")
        }
    }
}
...
//                   on BTN_INPUT_SOURCE
//
// NOTE:             If you are using th old API then use this event instead
//                   
// PURPOSE:          This button event is used to listen for input 
//                   on the touch panel and update the .
//
...
//		{
//			stack_var integer btn
//			stack_var char source[20]
//			
//			source = ''
//			btn = get_last(BTN_INPUT_SOURCE)
//			
//			if (length_string(source) > 0)
//				source = InputProps[nCurrentZone][btn].sSource
//			else
//				source = InputProps[nCurrentZone][btn].sLabel
//			
//			send_command vdvDev[nCurrentZone], "'INPUT-',source,',',itoa(InputProps[nCurrentZone][btn].nInput)"
//			println("'send_command ',dpstoa(vdvDev[nCurrentZone]),', ',39,'INPUT-',source,',',itoa(InputProps[nCurrentZone][btn].nInput),39")
//		}
//    }
//}
//---------------------------------------------------------------------------------
